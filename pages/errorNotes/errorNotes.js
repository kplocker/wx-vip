// pages/errorNotes/errorNotes.js
...
Page({
  ...
  onLoad: function (options) {
    ...
    db.collection('errorNotes').where({
      _openid: openId
    })
    .get()
    .then(res => {
  ...
  changePageData: function (currentQuestionCount, delay) {
    ...
    setTimeout(() => {
      ...
      for (var i = 0; i < that.data.questionItem.answer.length; i++) {
        console.log(that.data.questionItem.answer[i])
    ...
    wx.showModal({
      ...
      success: function (res) {
        ...
        if (res.confirm) {
          ...
          db.collection('errorNotes').doc(data._id)
            .remove()
            .then(res => {
              wx.showToast({
                title: '删除成功',
